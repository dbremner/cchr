...
\frame{\frametitle{Algoritme}
  ...
  \ForEach{TS $Z$}{FIRST[$Z$] $\leftarrow$ $\{Z\}$}
  \Repeat{FIRST, FOLLOW, nullable veranderen niet meer}{
    \ForEach{productie $X \rightarrow Y_1 Y_2 \cdots Y_k$}{
      \lFor{$i=1$ to $k$,}{\For{$j=i+1$ to $k$}{
        \If{alle $Y_i$ zijn nullable}{nullable[$X$] $\leftarrow$ true}
        \If{$Y_1 \cdots Y_{i-1}$ zijn nullable}{FIRST[$X$] $\leftarrow$ FIRST[$X$] $\cup$ FIRST[$Y_i$]}
        \If{$Y_{i+1} \cdots Y_k$ zijn nullable}{FOLLOW[$Y_i$] $\leftarrow$ FOLLOW[$Y_i$] $\cup$ FOLLOW[$X$]}
        \If{$Y_{i+1} \cdots Y_{j-1}$ zijn nullable}{FOLLOW[$Y_i$] $\leftarrow$ FOLLOW[$Y_i$] $\cup$ FIRST[$Y_j$]}
      }}
    }
  }
  \end{algorithm}}
