...
extern void *memccpy(void *__restrict __dest,
		     __const void *__restrict __src, int __c, size_t __n)
    __attribute__ ((__nothrow__)) __attribute__ ((__nonnull__(1, 2)));
...
extern unsigned long int __strtoul_internal(__const char *__restrict
    ...
    __attribute__ ((__nothrow__)) __attribute__ ((__nonnull__(1)));
__extension__
    extern long long int __strtoll_internal(__const char *__restrict
					    __nptr,
					    char **__restrict __endptr,
					    int __base, int __group)
    __attribute__ ((__nothrow__)) __attribute__ ((__nonnull__(1)));
...
__extension__ extern __inline unsigned int
    __attribute__ ((__nothrow__)) gnu_dev_major(unsigned long long int
						__dev)
{
...
__extension__ extern __inline unsigned int
    __attribute__ ((__nothrow__)) gnu_dev_minor(unsigned long long int
						__dev)
{
...
void static inline hash_t_init(hash_t * ht)
{
    ht->size = 3;
    ht->used = 0;
    ht->data = malloc(sizeof(hash_t_entry_t) * (1 << (ht->size + 1)));
    for (int j = 0; j < (1 << (ht->size + 1)); j++) {
	ht->data[j].used = 0;
}} hash_t_entry_t static inline *hash_t_find(hash_t * ht, uint32_t key)
{
    uint32_t h1 = hash1((key)), h2 = hash2((key));
    h1 = (h1 >> (32 - ht->size));
    h2 = (h2 >> (32 - ht->size)) | (1 << ht->size);
    if ((((key)) == (ht->data[h1].key)))
	return (&(ht->data[h1]));
    if ((((key)) == (ht->data[h2].key)))
	return (&(ht->data[h2]));
    return ((void *) 0);
} int static inline hash_t_have(hash_t * ht, uint32_t key)
{
    return ((hash_t_find(ht, key)) != ((void *) 0));
} uint32_t static inline *hash_t_get(hash_t * ht, uint32_t key)
{
    hash_t_entry_t *r = hash_t_find(ht, key);
    if (r == ((void *) 0))
	return ((void *) 0);
    return (&(r->val));
}
void static inline hash_t_set(hash_t * ht, uint32_t key, uint32_t val)
{
    hash_t_entry_t *r = hash_t_find(ht, key);
    if (r) {
	r->val = val;
	...
	h1 = (h1 >> (32 - ht->size));
	uint32_t key2 = (ht->data[h1].key);
	uint32_t val2 = (ht->data[h1].val);
	ht->data[h1].key = key;
	ht->data[h1].val = val;
	if (!ht->data[h1].used) {
	    ht->data[h1].used = 1;
	...
	uint32_t h2 = hash2((key2));
	h2 = (h2 >> (32 - ht->size)) | (1 << ht->size);
	uint32_t key3 = (ht->data[h2].key);
	uint32_t val3 = (ht->data[h2].val);
	ht->data[h2].key = key2;
	ht->data[h2].val = val2;
	if (!ht->data[h2].used) {
	    ht->data[h2].used = 1;
...
void static inline hash_t_double(hash_t * ht)
{
    hash_t_entry_t *nw =
	malloc(sizeof(hash_t_entry_t) * (4 << (ht->size)));
    for (int i = 0; i < (2 << ht->size); i++) {
	if (ht->data[i].used) {
	    uint32_t h =
		(i <
		 (1 << ht->size)) ? hash1((ht->data[i].key)) : hash2((ht->
								      data
								      [i].
								      key));
	    h = (h >> (31 - ht->size)) & 1;
...
int main(void)
{
    ...
    ((void)
     ((*hash_t_get(&ht, 1) ==
       2) ? 0 : (__assert_fail("*hash_t_get(&ht,1)==2", "<stdin>", 28,
    ...
    ((void)
     ((*hash_t_get(&ht, 1) ==
       2) ? 0 : (__assert_fail("*hash_t_get(&ht,1)==2", "<stdin>", 39,
			       __PRETTY_FUNCTION__), 0)));
    ((void)
     ((*hash_t_get(&ht, 2) ==
       3) ? 0 : (__assert_fail("*hash_t_get(&ht,2)==3", "<stdin>", 40,
			       __PRETTY_FUNCTION__), 0)));
    ((void)
     ((*hash_t_get(&ht, 3) ==
       5) ? 0 : (__assert_fail("*hash_t_get(&ht,3)==5", "<stdin>", 41,
			       __PRETTY_FUNCTION__), 0)));
    ((void)
     ((*hash_t_get(&ht, 4) ==
       7) ? 0 : (__assert_fail("*hash_t_get(&ht,4)==7", "<stdin>", 42,
			       __PRETTY_FUNCTION__), 0)));
    ((void)
     ((*hash_t_get(&ht, 5) ==
       11) ? 0 : (__assert_fail("*hash_t_get(&ht,5)==11", "<stdin>", 43,
				__PRETTY_FUNCTION__), 0)));
    ((void)
     ((*hash_t_get(&ht, 6) ==
       13) ? 0 : (__assert_fail("*hash_t_get(&ht,6)==13", "<stdin>", 44,
				__PRETTY_FUNCTION__), 0)));
    ((void)
     ((*hash_t_get(&ht, 7) ==
       17) ? 0 : (__assert_fail("*hash_t_get(&ht,7)==17", "<stdin>", 45,
				__PRETTY_FUNCTION__), 0)));
    ((void)
     ((*hash_t_get(&ht, 8) ==
       19) ? 0 : (__assert_fail("*hash_t_get(&ht,8)==19", "<stdin>", 46,
				__PRETTY_FUNCTION__), 0)));
    ((void)
     ((*hash_t_get(&ht, 9) ==
       23) ? 0 : (__assert_fail("*hash_t_get(&ht,9)==23", "<stdin>", 47,
				__PRETTY_FUNCTION__), 0)));
    ((void)
     ((*hash_t_get(&ht, 10) ==
       29) ? 0 : (__assert_fail("*hash_t_get(&ht,10)==29", "<stdin>", 48,
				__PRETTY_FUNCTION__), 0)));
    return 0;
}
